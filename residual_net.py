...
class ResBlock(nutszebra_chainer.Model):
    ...
    def __init__(self, in_channel, out_channel, n=38):
    ...
    def __call__(self, x, train=False):
        ...
        x = h + ResBlock.concatenate_zero_pad(x, h.data.shape, h.volatile, type(h.data))
...
class ResidualNetwork(nutszebra_chainer.Model):
    ...
    def __init__(self, category_num, block_num=3, out_channels=(16, 32, 64), N=(38, 38, 38)):
        ...
        for i, out_channel, n in six.moves.zip(six.moves.range(1, block_num + 1), out_channels, N):
            modules.append(('res_block{}'.format(i), ResBlock(in_channel, out_channel, n=n)))
